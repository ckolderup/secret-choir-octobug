...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  window.secretFields.forEach((fieldName) => {
    window.secretInputs[fieldName] = document.getElementById(
      `${fieldName}Input`
    );
  });
  ...
  secretsUpdate.addEventListener("click", async (e) => {
    ...
    window.secretFields.forEach((fieldName) => {
      switch (fieldName) {
        case "githubRepoUrl":
          const [_, githubRepoOwner, githubRepoName] = window.secretInputs[
            "githubRepoUrl"
          ]?.value?.match(/^https?\:\/\/github\.com\/([^\/]+)\/(.+)$/);
        ...
        default:
          localStorage.setItem(
            fieldName,
            window.secretInputs[fieldName]?.value
          );
      }
    });
